...
const VideoGeneration = ({ id, data, selected }) => {
  ...
  const handleRunSingleNode = async () => {
    ...
    try {
      ...
      // Resolve incoming connected edges dynamically
      const allNodes = (typeof getNodes === "function" ? getNodes() : nodes) || [];
      const incomingEdges = (edges || []).filter((e) => e.target === id);
      incomingEdges.forEach((edge) => {
        const sourceNode = allNodes.find((n) => n.id === edge.source);
        const sourceVal = sourceNode?.data?.resultUrl || sourceNode?.data?.outputs?.[0]?.value || sourceNode?.data?.formValues?.prompt;
        if (sourceVal && typeof sourceVal === "string" && sourceVal.trim()) {
          if (edge.targetHandle === "videoInput") {
            localSources.prompt = sourceVal;
          }
          if (edge.targetHandle === "videoInput2") {
            localSources.image_url = sourceVal;
            localSources.image = sourceVal;
          }
          if (edge.targetHandle === "videoInput3") {
            localSources.last_image = sourceVal;
          }
          if (edge.targetHandle === "videoInput4") {
            localSources.video_url = sourceVal;
          }
          if (edge.targetHandle === "videoInput5") {
            localSources.audio_url = sourceVal;
          }
          if (edge.targetHandle === "videoInput6") {
            const list = Array.isArray(localSources.images_list) ? [...localSources.images_list] : [];
            if (!list.includes(sourceVal)) list.push(sourceVal);
            localSources.images_list = list;
          }
          if (edge.targetHandle === "videoInput7") {
            const key = "video_files" in inputSchema ? "video_files" : "videos_list";
            const list = Array.isArray(localSources[key]) ? [...localSources[key]] : [];
            if (!list.includes(sourceVal)) list.push(sourceVal);
            localSources[key] = list;
          }
          if (edge.targetHandle === "videoInput8") {
            const key = "audio_files" in inputSchema ? "audio_files" : "audios_list";
            const list = Array.isArray(localSources[key]) ? [...localSources[key]] : [];
            if (!list.includes(sourceVal)) list.push(sourceVal);
            localSources[key] = list;
          }
      ...
      // Cross-field bridge: image_url <-> images_list
      let effectiveImagesList = Array.isArray(localSources.images_list) && localSources.images_list.length > 0
        ? localSources.images_list
        : (Array.isArray(localSources.images) && localSources.images.length > 0 ? localSources.images : []);
      let effectiveImageUrl = localSources.image_url || localSources.image || null;
      ...
      if ("images_list" in inputSchema || "images" in inputSchema || "image_urls" in inputSchema) {
        if (effectiveImagesList.length > 0) {
          localSources.images_list = effectiveImagesList;
          localSources.images = effectiveImagesList;
          localSources.image_urls = effectiveImagesList;
        }
      }
      if ("image_url" in inputSchema || "image" in inputSchema) {
        if (effectiveImageUrl) {
          localSources.image_url = effectiveImageUrl;
          localSources.image = effectiveImageUrl;
        }
      }

      // Cross-field bridge: video_url <-> videos_list
      let effectiveVideosList = Array.isArray(localSources.videos_list) && localSources.videos_list.length > 0
        ? localSources.videos_list
        : (Array.isArray(localSources.video_files) && localSources.video_files.length > 0 ? localSources.video_files : []);
      let effectiveVideoUrl = localSources.video_url || null;
      ...
      if ("videos_list" in inputSchema || "video_files" in inputSchema) {
        if (effectiveVideosList.length > 0) {
          localSources.videos_list = effectiveVideosList;
          localSources.video_files = effectiveVideosList;
        }
      }
      if ("video_url" in inputSchema) {
        if (effectiveVideoUrl) {
          localSources.video_url = effectiveVideoUrl;
        }
      }
      ...
      // Pre-validation for Image-to-Video models requiring input image
      const isI2V = selectedModel.id.includes("i2v") || selectedModel.id.includes("image-to-video") || (selectedModel.name || "").toLowerCase().includes("image to video") || (selectedModel.name || "").toLowerCase().includes("i2v");
      if (isI2V) {
        const hasImage = (params.images_list && params.images_list.length > 0) || params.image_url || params.image;
        if (!hasImage) {
          toast.error("Для модели Image-to-Video требуется исходное изображение. Подключите предыдущую ноду к входу изображения.");
          data.onDataChange(id, { isLoading: false });
          return;
        }
      }
