...
const TextGeneration = ({ id, data, selected }) => {
  ...
  const handleRunSingleNode = async () => {
    ...
    if (!runId) {
      toast.error(t("toasts.noRunId", {}, "No run_id available! Click 'Run All' button"));
      return;
    }
    ...
    try {
      ...
      if (!workflow_id) {
        toast.error(t("toasts.workflowSaveFailed", {}, "Failed to save workflow before running node"));
        data.onDataChange(id, { isLoading: false });
        return;
      }
      ...
      if (!modelSchema || !modelSchema.properties) {
        toast.error(t("toasts.noSchema", {}, "No input schema found for this model"));
        data.onDataChange(id, { isLoading: false });
        return;
      }
      ...
      incomingEdges.forEach((edge) => {
        ...
        if (sourceVal && typeof sourceVal === "string" && sourceVal.trim()) {
          if (edge.targetHandle === "textInput") {
            localSources.prompt = sourceVal;
          }
          if (edge.targetHandle === "textInput2") {
            localSources.image_url = sourceVal;
            localSources.image = sourceVal;
          }
          if (edge.targetHandle === "textInput3") {
          ...
          if (edge.targetHandle === "textInput4") {
            localSources.system_prompt = sourceVal;
          }
        }
      });
      ...
      try {
        ...
        console.warn("Could not calculate dynamic cost before running text node:", err);
      }
      ...
      data.onDataChange(id, { isLoading: false });
      toast.error(error.response?.data?.detail || t("toasts.errorRunningNode", {}, "Error running node"));
      console.error(error);
    };
  };
  ...
  const handleDeleteNode = () => {
    ...
    toast.success(t("toasts.nodeDeleted", { id }, `Deleted node ${id}`));
  };
          ...
          <span className="text-xs text-blue-500 -mt-0.5 font-medium flex items-center gap-1 opacity-80">
            {isRefreshingCost ? (
              ...
              <span title={generationCost !== null ? `$${generationCost} USD` : ""}>
                {generationCost === 0 ? t("builder.free", {}, "Free") : `🪙 ${generationCostTokens ?? (generationCost * 100)} токенов`}
              </span>
            )}
