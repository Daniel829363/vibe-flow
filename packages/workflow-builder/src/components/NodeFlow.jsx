...
const processWorkflowData = (workflowData, nodeSchemas, id) => {
  ...
  return {
    ...
    metadata: {
      ...
      interactionMode: workflowData.is_owner !== undefined ? workflowData.is_owner : true,
...
const NodeFlow = ({ initialNodeSchemas, initialWorkflowData }) => {
  ...
  const [interactionMode, setInteractionMode] = useState(initialState?.metadata?.interactionMode !== undefined ? initialState.metadata.interactionMode : true);
  ...
  const [tokenRate, setTokenRate] = useState(100);
  ...
  const handleNodesChange = useCallback((changes) => {
    ...
    const hasMeaningfulChanges = changes.some(c => 
      (c.type === 'position' && (c.dragging !== undefined || c.position !== undefined)) ||
      c.type === 'remove' ||
      c.type === 'add' ||
      c.type === 'dimensions'
    );
  ...
  const loadPreset = (preset) => {
    ...
    const nodesWithSchemas = (preset.nodes || []).map(n => ({
      ...n,
      data: {
        ...n.data,
        nodeSchemas,
    ...
    setNodes(nodesWithSchemas);
    setEdges(preset.edges || []);
    setTimeout(() => fitView({ padding: 0.4, duration: 500, minZoom: 0.2 }), 100);
  };
  ...
  const restoreWorkflow = useCallback((workflowData) => {
    ...
    setInteractionMode(workflowData.is_owner !== undefined ? workflowData.is_owner : true);
  ...
  useEffect(() => {
    ...
    axios.get(`/api/workflow/get-workflow-def/${id}`)
      ...
      .catch((error) => {
        console.log("Failed to load remote workflow:", error);
  ...
  const onDataChange = (id, newData, targetNodeId = null) => {
    if (newData.formValues || newData.selectedModel || newData.exposedHandles) {
      setIsDirty(true);
    }
    setNodes((prevNodes) => {
  ...
  const buildWorkflowPayload = () => {
    const nodeData = nodes.map((node) => {
      ...
      // Cross-field bridge for images (image_url <-> images_list)
      let effectiveImagesList = Array.isArray(dynamicImagesList) && dynamicImagesList.length > 0
        ? dynamicImagesList
        : [];
      let effectiveImageUrl = dynamicImageUrl;

      if (effectiveImagesList.length === 0 && effectiveImageUrl) {
        effectiveImagesList = [effectiveImageUrl];
      }
      if (!effectiveImageUrl && effectiveImagesList.length > 0) {
        effectiveImageUrl = effectiveImagesList[0];
      }

      // Cross-field bridge for videos (video_url <-> videos_list)
      let effectiveVideosList = Array.isArray(dynamicVideosList) && dynamicVideosList.length > 0
        ? dynamicVideosList
        : [];
      let effectiveVideoUrl = dynamicVideoUrl;

      if (effectiveVideosList.length === 0 && effectiveVideoUrl) {
        effectiveVideosList = [effectiveVideoUrl];
      }
      if (!effectiveVideoUrl && effectiveVideosList.length > 0) {
        effectiveVideoUrl = effectiveVideosList[0];
      }
      ...
      const localSources = {
        ...
        images_list: effectiveImagesList,
        images: effectiveImagesList,
        image_urls: effectiveImagesList,
        image_url: effectiveImageUrl,
        image: effectiveImageUrl,
        video_url: effectiveVideoUrl,
        audio_url: dynamicAudioUrl,
        last_image: dynamicLastImage,
        videos_list: effectiveVideosList,
        video_files: effectiveVideosList,
      ...
      if (node.type === "apiNode") {
        for (const [key, meta] of Object.entries(wavespeedSchema || {})) {
          if (localSources[key] !== undefined && localSources[key] !== null) {
            if (Array.isArray(localSources[key])) {
              if (localSources[key].length > 0) params[key] = localSources[key];
            } else {
              params[key] = localSources[key];
            }
          } else if (meta?.default !== undefined && meta?.default !== null) {
            if (!Array.isArray(meta.default) || meta.default.length > 0) {
        ...
        for (const [key, meta] of Object.entries(filteredInputParams)) {
          if (localSources[key] !== undefined && localSources[key] !== null) {
            if (Array.isArray(localSources[key])) {
              if (localSources[key].length > 0) params.params[key] = localSources[key];
            } else {
              params.params[key] = localSources[key];
            }
          } else if (meta?.default !== undefined && meta?.default !== null) {
            if (!Array.isArray(meta?.default) || meta.default.length > 0) {
              params.params[key] = meta.default;
            }
          }
        }
  ...
  const handleSaveWorkFlow = async () => {
    ...
    try {
      ...
      const newId = response.data.workflow_id;
      setDropDown(0);
      setWorkflowIds(newId, runId);
      setWorkflowId(newId);
      setIsDirty(false);
      setIsSaving(false);
      ...
      toast.success(t("builder.saveSuccess", {}, "Workflow saved successfully"));
      return newId;
    } catch (error) {
      console.log(error);
      setIsSaving(false);
      if (error.response) {
        toast.error(`Failed: ${error.response.data.detail || error.response.data.error || "Server error"}`);
      } else {
  ...
  const handleDuplicateWorkflow = async () => {
    ...
    try {
      const response = await axios.post(`/api/workflows/${workflowId}/copy`);
      const newRemoteId = response.data?.workflow?.remote_workflow_id || response.data?.workflow?.id;
      toast.success(t("builder.duplicateSuccess", {}, "Копия успешно создана!"));
      if (newRemoteId) {
        window.location.href = `/workflow/${newRemoteId}`;
      } else {
        setIsRunning(0);
      }
    } catch (error) {
        ...
        .then((response) => {
          ...
          Object.entries(nodesStatus).forEach(([id, runs]) => {
            ...
            if (status === "succeeded" || status === "completed") {
              ...
              const failedMsg = latestRun?.result?.outputs?.[0]?.value?.error || latestRun?.result?.error || latestRun?.error || latestRun?.detail || "Generation Failed";
              setNodes((prevNodes) => prevNodes.map(n => {
                if (n.id === id) {
                  return { ...n, data: { ...n.data, isLoading: false, errorMsg: failedMsg } };
                }
                return n;
              }));
            }
          ...
          if (allCompleted) {
            ...
            const failedReasons = [];
            Object.entries(nodesStatus).forEach(([nId, nodeRuns]) => {
              if (nodeRuns && nodeRuns[0]?.status === "failed") {
                const latestR = nodeRuns[0];
                const msg = latestR?.result?.outputs?.[0]?.value?.error || latestR?.result?.error || latestR?.error || latestR?.detail;
                if (msg) failedReasons.push(`${nId}: ${msg}`);
              }
            });
            const toastMsg = failedReasons.length > 0 
              ? `Workflow failed: ${failedReasons.join("; ")}`
              : "Workflow failed on some nodes";
  ...
  const handleRunWorkflow = async () => {
    ...
    try {
      ...
      setLoadingNodes({});
      const savedWorkflowId = await handleSaveWorkFlow();
      const targetWorkflowId = savedWorkflowId || workflowId;

      if (!targetWorkflowId) {
        toast.error("No workflow ID available to run");
        setIsRunning(0);
        return;
      }
      ...
      setRunId(newRunId);
      setWorkflowIds(targetWorkflowId, newRunId);
  ...
  const runNodeFromFlow = (nodeId) => {
    if (!interactionMode) {
      toast.error(t("builder.viewOnlyNotice", {}, "Режим только для чтения. Создайте копию для запуска."));
      return;
    }
    setNodes((nds) =>
      nds.map((n) =>
  ...
  }, [nodes, setNodes, interactionMode]);

  const nodesWithHandlers = nodes.map((node) => ({
    ...node,
    data: {
      ...node.data,
      isReadOnly: !interactionMode,
    ...
    <div tabIndex={0} onKeyDown={onKeyDown} className="flex h-dvh w-full relative">
      {isRestoring && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#030303] text-white selection:bg-blue-500/30">
          ...
          <div className="relative z-10 flex flex-col items-center gap-5 p-8 rounded-3xl bg-[#0a0a0a]/90 border border-white/10 backdrop-blur-2xl shadow-2xl max-w-sm w-full mx-4 text-center">
            ...
            <div className="flex flex-col gap-1.5">
              <h3 className="text-base font-bold text-white tracking-tight">
                {t("builder.loadingWorkflow", {}, "Загрузка рабочего процесса")}
              </h3>
              <p className="text-xs text-zinc-400 font-medium">
                {t("builder.initializingCanvas", {}, "Инициализация холста и загрузка компонентов...")}
              </p>
            </div>
      ...
      {/* ── TOP HEADER / NAVBAR ────────────────────────────────────────────── */}
      <header className="flex items-center justify-between absolute top-0 z-30 bg-[#030303]/90 backdrop-blur-xl w-full px-4 md:px-8 py-3 border-b border-white/10 shadow-2xl">
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          {/* Back button */}
          <Link
            href="/workflow"
            className="flex items-center justify-center w-8 h-8 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-400 hover:text-white transition-all shadow-sm shrink-0"
            title={t("common.back", {}, "Back to Workflows")}
          >
            <FaAngleLeft size={14} />
          </Link>
          ...
          <Link href="/" className="flex items-center gap-2 font-bold text-base hover:opacity-90 transition-opacity shrink-0">
            <div className="w-7 h-7 bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg flex items-center justify-center shadow-[0_0_12px_rgba(37,99,235,0.4)]">
              <GoWorkflow className="text-white" size={15} />
            </div>
            <span className="text-white font-bold hidden sm:inline text-sm">
              Workflow<span className="text-blue-500">Pro</span>
            </span>
          </Link>

          <div className="h-4 w-[1px] bg-white/10 mx-0.5 hidden sm:block shrink-0" />
          ...
          <button
            ...
            className={`flex items-center gap-2 text-xs sm:text-sm font-bold text-zinc-300 hover:text-white px-2.5 py-1 rounded-lg hover:bg-white/5 transition-all truncate border border-transparent hover:border-white/10 ${!interactionMode ? "cursor-default" : "cursor-pointer"}`}
            title={interactionMode ? "Click to rename" : "Только просмотр"}
          >
            <span className="truncate max-w-[140px] sm:max-w-[240px] md:max-w-[360px]">
              {workflowName ? workflowName : t("builder.untitled", {}, "Untitled")}
            </span>
            {interactionMode && <FaRegEdit size={12} className="text-zinc-500 shrink-0" />}
          </button>

          {!interactionMode && (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-bold tracking-wide">
              <MdLockOutline size={13} />
              <span>{t("builder.viewOnly", {}, "Только просмотр")}</span>
            </span>
          )}

          {workflowCategory && workflowCategory !== "General" && (
            <span className="hidden md:inline-flex text-[10px] px-2 py-0.5 rounded-md bg-white/5 text-zinc-400 border border-white/10 font-semibold tracking-wide">
              {workflowCategory}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {/* Settings / Template menu (only in edit mode) */}
          {interactionMode && (
            <div
              className="relative"
              onBlur={(e) => {
            ...
            >
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 border border-white/10 bg-white/5 text-zinc-300 hover:text-white hover:bg-white/10 text-xs font-semibold rounded-full transition-all cursor-pointer"
              >
                <FaToolbox size={12} />
                <span className="hidden sm:inline">{t("builder.settings", {}, "Settings")}</span>
                <FaAngleDown size={10} className={`transition-transform duration-300 ${isSettingsOpen ? "rotate-180" : ""}`} />
              </button>

              {isSettingsOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-[#0d0e11] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden py-1">
                  <button
                    type="button"
                    suppressHydrationWarning={true}
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-xs text-zinc-300 hover:bg-white/5 hover:text-white transition-colors border-b border-white/5 disabled:opacity-50 cursor-pointer"
                  >
                    {isRunning === 4 ? (
                    ...
                    <span>{template.isPublishedTemplate ? t("builder.undoTemplate", {}, "Undo Template") : t("builder.makeTemplate", {}, "Make Template")}</span>
                  </button>
                  <button
                    type="button"
                    suppressHydrationWarning={true}
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-xs text-zinc-300 hover:bg-white/5 hover:text-white transition-colors cursor-pointer"
                  >
                    <RiInputMethodLine size={14} />
                    <span>{t("builder.category", {}, "Category")}</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {interactionMode ? (
            <>
              ...
              <button
                type="button"
                suppressHydrationWarning={true}
                disabled={isSaving || !interactionMode}
                onClick={handleSaveWorkFlow}
                title={isDirty ? t("builder.unsavedChanges", {}, "Unsaved changes") : t("builder.saved", {}, "Saved")}
                className={`hidden sm:flex items-center gap-1.5 px-3.5 py-1.5 border text-xs font-semibold rounded-full transition-all disabled:opacity-50 cursor-pointer ${
                  isDirty
                    ? "border-blue-500/40 bg-blue-600/20 text-blue-300 hover:bg-blue-600/30 hover:text-white shadow-[0_0_12px_rgba(59,130,246,0.3)]"
              ...
              >
                {isSaving ? (
                  ...
                  <>
                    ...
                    <MdSave size={14} className="text-blue-400" />
                    <span className="font-bold text-white">{t("builder.save", {}, "Save")}</span>
                  </>
                ) : (
                  <>
                    <FaCheck size={11} className="text-emerald-400" />
                    <span className="text-zinc-300">{t("builder.saved", {}, "Saved")}</span>
                  </>
                )}
              </button>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-full transition-all shadow-[0_0_15px_rgba(37,99,235,0.4)] hover:shadow-[0_0_20px_rgba(37,99,235,0.6)] disabled:opacity-50 active:scale-95 cursor-pointer whitespace-nowrap"
              >
                {isRunning === 1 ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-t-transparent border-white rounded-full animate-spin"></div>
                    <span>{t("builder.running", {}, "Running...")}</span>
                  </>
                ) : (
                  <>
                    <FaPlay size={11} />
                    <span>{t("builder.runAll", {}, "Run All")}</span>
                    {parseFloat(totalWorkflowCost) > 0 && (
                      <span className="px-1.5 py-0.5 bg-black/40 rounded-md text-[10px] font-mono text-amber-300" title={`$${totalWorkflowCost} USD`}>
                        🪙 {(parseFloat(totalWorkflowCost) * tokenRate).toFixed(1)} токенов
                      </span>
                    )}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-full transition-all shadow-[0_0_15px_rgba(37,99,235,0.4)] hover:shadow-[0_0_20px_rgba(37,99,235,0.6)] active:scale-95 cursor-pointer whitespace-nowrap"
            >
              {isRunning === 3 ? (
                <>
                  <div className="w-3.5 h-3.5 border-2 border-t-transparent border-white rounded-full animate-spin"></div>
                  <span>{t("builder.duplicating", {}, "Копирование...")}</span>
                </>
              ) : (
                <>
                  <IoDuplicateOutline size={15} />
                  <span>{t("builder.duplicateWorkflow", {}, "Создать копию")}</span>
                </>
              )}
            </button>
          )}
      ...
      <div className={`absolute left-4 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-2 bg-[#0a0a0a]/90 backdrop-blur-xl p-1.5 rounded-2xl border border-white/10 shadow-2xl ${isRestoring && "hidden"}`}>
        <button
          type="button"
          suppressHydrationWarning={true}
          onClick={() => toast.error(t("builder.lockedWorkflow", {}, "This workflow can't be edited."))}
          className={`p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-zinc-400 hover:text-white transition ${interactionMode && "hidden"}`}
          title="Workflow is locked"
        >
        ...
        >
          <button
            ...
            title="Add Node"
            className={`p-2.5 rounded-xl cursor-pointer outline-none transition-all ${dropDown === 1 ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "text-zinc-300 hover:text-white hover:bg-white/10"}`}
          >
            <FaPlus size={16} />
          </button>
          {dropDown === 1 && (
        ...
        >
          <button
            ...
            title="Utility Nodes"
            className={`p-2.5 rounded-xl cursor-pointer outline-none transition-all ${dropDown === 4 ? "bg-blue-600 text-white shadow-lg shadow-blue-600/30" : "text-zinc-300 hover:text-white hover:bg-white/10"}`}
          >
            <FaToolbox size={16} />
          </button>
          {dropDown === 4 && (
            <div className="absolute left-14 top-1/2 -translate-y-1/2 bg-[#0d0e11] border border-white/10 p-3 rounded-2xl shadow-2xl flex flex-col gap-2 w-56 z-50">
              <h3 className="w-full text-center text-xs font-bold uppercase tracking-wider text-zinc-400">{t("builder.utilityNode", {}, "Utility Node")}</h3>
              <div className="flex flex-col gap-1.5 w-full">
                <button
                  type="button"
                  suppressHydrationWarning={true}
                  onClick={() => addNode("concatNode", null, { selectedModel: concatModels[0] })}
                  className="flex gap-2.5 items-center py-2 px-3 text-zinc-200 hover:text-white text-xs font-semibold cursor-pointer bg-white/5 rounded-xl hover:bg-white/10 transition-colors"
                >
                  <TbArrowMerge className="rotate-90 text-blue-400" size={16} />
                  <span>{t("builder.promptConcatenator", {}, "Prompt Concatenator")}</span>
                </button>
                <button
                  type="button"
                  suppressHydrationWarning={true}
                  ...
                  className="flex gap-2.5 items-center py-2 px-3 text-zinc-200 hover:text-white text-xs font-semibold cursor-pointer bg-white/5 rounded-xl hover:bg-white/10 transition-colors"
                >
                  <TbArrowMerge className="rotate-90 text-orange-400" size={16} />
                  <span>{t("builder.videoCombiner", {}, "Video Combiner")}</span>
                </button>
              </div>
            </div>
          )}
      ...
      <div className={`absolute bottom-5 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 bg-[#0a0a0a]/90 backdrop-blur-xl px-4 py-2 rounded-2xl border border-white/10 shadow-2xl ${isRestoring && "hidden"}`}>
        {/* Info stats */}
        <div className="flex items-center gap-2.5 text-xs text-zinc-400 font-medium pr-3 border-r border-white/10">
          <span className="flex items-center gap-1.5 text-zinc-300 font-semibold">
            <GoWorkflow size={14} className="text-blue-400" />
            <strong className="text-white">{nodes.length}</strong> {t("builder.nodesCount", {}, "нод")}
          </span>
          <span className="text-zinc-600">•</span>
          <span className="text-zinc-400 font-medium">
            <strong className="text-zinc-300">{edges.length}</strong> {t("builder.edgesCount", {}, "связей")}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            title={isDragging ? "Switch to Select mode" : "Switch to Pan mode"}
            className={`p-1.5 rounded-lg transition cursor-pointer ${!isDragging ? "bg-blue-600 text-white" : "text-zinc-400 hover:bg-white/10 hover:text-white"}`}
          >
            <LuMousePointer2 size={15} />
          </button>
        </div>

        {/* Total Cost Badge */}
        {parseFloat(totalWorkflowCost) > 0 && (
          <div className="flex items-center gap-1.5 pl-3 border-l border-white/10 text-xs" title={`$${totalWorkflowCost} USD`}>
            <span className="text-zinc-500 font-medium">Расход:</span>
            <span className="text-amber-400 font-bold font-mono">🪙 {(parseFloat(totalWorkflowCost) * tokenRate).toFixed(1)} токенов</span>
          </div>
        )}
        ...
        <div className="absolute right-2 top-16 z-50 w-80 h-full max-h-[90%] bg-[#09090b]/80 backdrop-blur-xl border border-white/20 rounded-2xl flex transition-all duration-300 ease-in-out shadow-2xl">
          ...
          <div className="flex flex-col gap-4 h-full w-full">
              ...
              <div className="flex flex-col gap-4 w-full h-full">
                ...
                >
                  ...
                  <button
                    ...
                    disabled={!interactionMode}
                    onClick={() => interactionMode && setDropDown(prev => prev === 3 ? 0 : 3)}
                    className={`flex items-center justify-between gap-1 text-sm text-center text-white w-full h-full whitespace-nowrap px-3 py-2 bg-zinc-900/50 border border-white/10 rounded-lg transition-all ${
                      !interactionMode ? "opacity-60 cursor-default" : "hover:border-white/20 cursor-pointer focus:outline-none"
                    }`}
                  >
                      ...
                      <div className="flex flex-col overflow-y-auto">
                        {getFilteredModelsForNode(selectedNode).length > 0 ? (
                          ...
                          <p className="text-xs text-gray-400 text-center py-2">
                            {t("navbar.noModelsFound", {}, "No models found")}
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  (() => {
                    ...
                    return selectedNode?.data?.loading === 1 ? (
                      <div className="flex flex-col items-center justify-center gap-2 h-full w-full">
                        ...
                        <span className="text-xs text-white">{t("builder.generating", {}, "Fetching model...")}</span>
                      </div>
                        ...
                        {interactionMode && (
                          ...
                          >
                            {selectedNode?.data?.loading === 1 ? (
                              <><div className="w-3 h-3 rounded-full border border-t-transparent group-hover:border-t-transparent border-black group-hover:border-white animate-spin"></div>{t("builder.generating", {}, "Generating...")}</>
                            ) : (
                              <>{t("builder.fetchModel", {}, "Fetch Model")}</>
                            )}
                          </button>
                        )}
                        {Object.entries(selectedNode?.data?.taskData || {}).map(([key, meta], idx) => {
                          ...
                          return (
                            <RenderApiField
                              ...
                              onToggleHandle={isHardcoded || !interactionMode ? null : (field) => {
                      ...
                      Object.entries(inputSchema?.properties || inputSchema).map(([key, meta], idx) => {
                        ...
                        return (
                          <RenderField
                            ...
                            handleChange={interactionMode ? updateNodeFromPanel : () => {}}
                            data={inputSchema}
                      ...
                      <div className="text-center py-8">
                        <p className="text-sm text-gray-400">{t("builder.noProperties", {}, "No properties available")}</p>
                      </div>
                    );
                  })()
                ) : (
                  <div className="text-center py-8">
                    <p className="text-sm text-gray-400">{t("builder.selectModelFirst", {}, "Please select a model first")}</p>
                  </div>
                )}
              </div>
            </div>
            <div className="p-4 flex flex-col gap-3">
              {/* Make Output Toggle */}
              <label className={`flex items-center justify-between ${interactionMode ? "cursor-pointer" : "cursor-default opacity-60"} group`}>
                <span className="text-xs text-gray-300 font-medium">{t("builder.markAsOutput", {}, "Mark as Output")}</span>
              ...
              {!selectedNode?.data?.selectedModel?.id?.includes("passthrough") && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (selectedNode) runNodeFromFlow(selectedNode.id);
                  }}
                  disabled={!interactionMode || loadingNodes[selectedNode.id]}
                  className={`text-sm font-semibold flex items-center justify-center gap-2 rounded-lg px-4 py-2 border transition-all ${
                    !interactionMode
                      ? "bg-zinc-800/80 text-zinc-400 border-white/5 cursor-not-allowed opacity-70"
                      : "text-white bg-blue-500 border-blue-500/50 hover:bg-blue-600 cursor-pointer shadow-lg shadow-blue-900/20 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                  } w-full`}
                >
      ...
      <div
        className={`fixed inset-0 flex flex-col items-center justify-center z-50 overflow-auto bg-black/80 backdrop-blur-xl transition-all duration-200 ease-in-out p-4 ${
          dropDown === 2 ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible pointer-events-none"
        }`}
        onClick={() => setDropDown(0)}
      >
        <div className="bg-[#0d0e11] border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl flex flex-col gap-5" onClick={(e) => e.stopPropagation()}>
          <div className="text-center">
            <h3 className="text-base font-black uppercase tracking-wider text-white">
              {t("builder.renameModalTitle", {}, "Переименовать процесс")}
            </h3>
          </div>
          <div className="flex flex-col gap-2 w-full">
            <label className="text-xs font-semibold text-zinc-400">Название процесса</label>
            <input
              type="text"
              value={workflowName}
              autoFocus
              ...
              className="bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 transition-all w-full font-medium"
              onKeyDown={async (e) => {
                if (e.key === "Enter") {
                  setDropDown(0);
                  if (workflowId) {
                    try {
                      await axios.post(`/api/workflow/update-name/${workflowId}`, { name: workflowName });
                    } catch (err) {
                      console.warn("Failed to update name via update-name:", err);
                    }
                  }
          ...
          <div className="flex items-center w-full gap-3 pt-1">
            <button
              ...
              suppressHydrationWarning={true}
              onClick={() => setDropDown(0)}
              className="flex-1 py-2.5 px-4 rounded-xl text-zinc-400 hover:text-white hover:bg-white/5 font-bold text-xs uppercase tracking-wider transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="flex-1 py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-lg cursor-pointer"
            >
      ...
      {nodes.length === 0 && !isPresetsDismissed && interactionMode && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/60 backdrop-blur-md p-6">
          <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-300 max-w-5xl w-full max-h-[90%] overflow-y-auto custom-scrollbar p-6 bg-[#0a0a0a]/90 border border-white/10 rounded-3xl shadow-2xl">
            <div className="text-center">
              <h2 className="text-2xl font-black text-white tracking-tight uppercase">
                {t("builder.selectWorkflowTitle", {}, "Выберите шаблон процесса")}
              </h2>
              <p className="text-xs text-zinc-400 font-semibold tracking-wider mt-1">
                {t("builder.selectWorkflowSubtitle", {}, "Или начните с чистого холста")}
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 w-full">
              {presets.map((preset) => (
                <button
                  ...
                  className="group relative flex flex-col bg-white/[0.02] hover:bg-white/[0.06] aspect-[4/3] border border-white/10 hover:border-blue-500/50 rounded-2xl shadow-xl hover:shadow-2xl hover:scale-105 cursor-pointer transition-all duration-300 overflow-hidden text-left"
                >
                  <div className="z-10 p-2.5 bg-black/40 border-b border-white/5 flex items-center px-3 justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${preset.id === "empty-workflow" ? "bg-zinc-400" : "bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)]"}`} />
                      <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">
                        {preset.id === "empty-workflow" ? "НОВЫЙ" : "ШАБЛОН"}
                      </span>
                    </div>
                  </div>
                  <div className="z-0 p-3.5 flex flex-col gap-2 h-full justify-between">
                    <div className="flex items-center gap-2.5 z-10">
                      <div className="text-blue-400 group-hover:scale-110 transition-transform">
                        {iconMap[preset.icon] || <RiInputMethodLine size={16} />}
                      </div>
                      <h3 className="text-xs font-bold text-white leading-tight group-hover:text-blue-400 transition-colors">
                        {preset.title}
                      </h3>
                    </div>
                    {preset.image && (
                      <div className="absolute inset-0 z-0 w-full h-full overflow-hidden">
                        <img src={preset.image} alt="" className="w-full h-full object-cover opacity-30 group-hover:opacity-60 transition-opacity duration-300" />
                        <div className="absolute inset-0 z-10 w-full h-full bg-gradient-to-t from-black via-black/50 to-transparent" />
                      </div>
                    )}
                    {preset.description && (
                      <p className="z-10 text-[10px] text-zinc-400 leading-relaxed line-clamp-2 mt-auto">
                        {preset.description}
                      </p>
                    )}
                  </div>
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-full bg-white/5 hover:bg-white/10 text-xs text-zinc-400 hover:text-white font-bold uppercase tracking-wider transition-all border border-white/10 hover:border-white/20 cursor-pointer"
            >
              {t("builder.startEmptyCanvas", {}, "Пустой холст")}
            </button>
          </div>
        </div>
      )}
