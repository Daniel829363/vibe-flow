    ...
    try {
      ...
      incomingEdges.forEach((edge) => {
        ...
        if (sourceVal && typeof sourceVal === "string" && sourceVal.trim()) {
          ...
          const isList = listFields.includes(edge.targetHandle) || inputSchema?.[edge.targetHandle]?.type === "array";
          if (isList) {
            const list = Array.isArray(localSources[edge.targetHandle]) ? [...localSources[edge.targetHandle]] : [];
            if (!list.includes(sourceVal)) list.push(sourceVal);
            localSources[edge.targetHandle] = list;
          } else {
            localSources[edge.targetHandle] = sourceVal;
          }
        }
      });
      ...
      const filteredInputParams = Object.fromEntries(
        Object.entries(localSources).filter(([key]) =>
