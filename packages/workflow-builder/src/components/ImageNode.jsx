      ...
      .then((response) => {
        ...
        if (latest.status === "failed") {
          ...
          if (outputs && outputs[0]?.value?.error) {
            ...
            errorMsg = latest.result.error;
          } else if (latest?.error) {
            errorMsg = latest.error;
          } else if (latest?.detail) {
            errorMsg = latest.detail;
          }
    ...
    try {
      ...
      incomingEdges.forEach((edge) => {
        ...
        if (sourceVal && typeof sourceVal === "string" && sourceVal.trim()) {
          if (["imageInput", "textInput"].includes(edge.targetHandle)) {
            localSources.prompt = sourceVal;
          }
          if (["imageInput2", "textInput3"].includes(edge.targetHandle)) {
          ...
          if (["imageInput3", "textInput2"].includes(edge.targetHandle)) {
      ...
      const hasImageListInSchema = "images_list" in inputSchema || "images" in inputSchema || "image_urls" in inputSchema;
      const hasImageUrlInSchema = "image_url" in inputSchema || "image" in inputSchema;
      ...
      if (hasImageListInSchema && effectiveImagesList.length > 0) {
      ...
      if (hasImageUrlInSchema && effectiveImageUrl) {
      ...
      // Pre-validation for Image-to-Image / Edit models requiring input images
      const isI2I =
        selectedModel.id.includes("i2i") ||
        selectedModel.id.includes("image-to-image") ||
        selectedModel.id.includes("image-edit") ||
        selectedModel.id.includes("edit") ||
        (selectedModel.name || "").toLowerCase().includes("image to image") ||
        (selectedModel.name || "").toLowerCase().includes("image edit") ||
        (selectedModel.name || "").toLowerCase().includes("edit") ||
        (selectedModel.name || "").toLowerCase().includes("i2i");

      if (isI2I) {
        const hasImage =
          (params.images_list && params.images_list.length > 0) ||
          (params.images && params.images.length > 0) ||
          (params.image_urls && params.image_urls.length > 0) ||
          params.image_url ||
          params.image;
        if (!hasImage) {
          toast.error("Для модели редактирования изображений требуется исходное изображение. Подключите предыдущую ноду к входу изображения или загрузите его.");
          data.onDataChange(id, { isLoading: false });
          return;
        }
      }
      ...
      try {
        ...
        console.warn("Could not calculate dynamic cost before running image node:", err);
      }
