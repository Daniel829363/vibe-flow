...
const UploadNode = ({ id, data, formValues, setFormValues, selectedModel, loading, uploadType, acceptType }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    if (!acceptedTypes.includes(file.type)) {
      toast.error(t("toasts.validFile", { type: acceptType }, `Please upload a valid ${acceptType} file`));
      return;
    };
    ...
    axios.post("/api/app/upload_file", formData, {
    ...
    .catch((error) => {
      ...
      toast.error(t("toasts.uploadFailed", {}, errMsg));
      setUploading(false);
      setUploadProgress(0);
    });
  };
      ...
      <div className="flex flex-col items-center justify-center w-full h-full flex-1">
        {uploadType === "text" ? (
          <textarea
            ...
            placeholder={t("nodes.prompt", {}, "Enter your text prompt here...")}
          ...
          >
            {uploading ? (
              <div className="flex flex-col justify-center gap-2 w-full h-full max-w-[95%]">
                <h4 className="text-xs text-white">{t("common.loading", {}, "Uploading...")} {uploadProgress}%</h4>
              ...
              >                <FiUpload size={20} />
                <span className="text-xs capitalize">{t("nodes.uploadMedia", {}, "Upload Media")} ({acceptType})</span>
                <span className="text-xs text-gray-500">{t("nodes.dropFilesPrompt", {}, "Drop image, video, or audio files here")}</span>
                <input
                  type="file"
