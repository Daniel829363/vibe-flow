...
const NodeOptionsMenu = ({ 
  ...
  // In read-only mode, if there is nothing to show (e.g. no downloadUrl), hide the options button
  if (isReadOnly && !downloadUrl) {
    return null;
  }
        ...
        <div className="absolute right-0 mt-2 whitespace-nowrap bg-[#1b1e23]/95 backdrop-blur-xl border border-white/10 rounded-md shadow-2xl z-[100] overflow-hidden animate-in fade-in zoom-in duration-200">
          {!isReadOnly && onDuplicate && (
            ...
            >
              ...
              <span>{t("nodes.duplicateNode", {}, "Duplicate")}</span>
            </button>
          )}
          ...
          {downloadUrl && (
            ...
            >
              ...
              <span>{t("nodes.download", {}, "Download")}</span>
            </button>
          )}

          {!isReadOnly && showThumbnailOption && (
            <button
              type="button"
              suppressHydrationWarning={true}
              onClick={(e) => {
            ...
            >
              ...
              <span>{t("nodes.setThumbnail", {}, "Set Thumbnail")}</span>
            </button>
          )}
          ...
          {!isReadOnly && onDelete && (
            ...
            >
              ...
              <span>{t("nodes.deleteNode", { id: nodeId }, `Delete Node ${nodeId}`)}</span>
            </button>
          )}
        </div>
      )}
