...
export default function SettingsModal({ workflow, onClose, onVisibilityChange, onOpenCoverModal }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(
        visibility === "public"
          ? "Процесс теперь публичный"
          : "Процесс теперь приватный"
      );
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
          <div>
            <h3 className="text-white font-black text-sm uppercase tracking-widest">Настройки процесса</h3>
        ...
        <div className="p-6 space-y-4">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3">Видимость</p>
          ...
          >
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
              visibility === "private" ? "bg-blue-600/20 text-blue-400" : "bg-white/5 text-zinc-500"
            }`}>
              <HiOutlineLockClosed size={18} />
            </div>
            <div>
              <p className="text-white font-bold text-sm">Приватный</p>
              <p className="text-zinc-500 text-xs mt-0.5">Доступен только вам и тем, кому вы выдали доступ</p>
            </div>
            <div className={`w-4 h-4 rounded-full border-2 ml-auto mt-1 flex-shrink-0 ${
              visibility === "private" ? "border-blue-500 bg-blue-500" : "border-zinc-600"
            }`} />
          </button>
          ...
          >
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
              visibility === "public" ? "bg-green-600/20 text-green-400" : "bg-white/5 text-zinc-500"
            }`}>
              <HiOutlineGlobeAlt size={18} />
            </div>
            <div>
              <p className="text-white font-bold text-sm">Публичный</p>
              <p className="text-zinc-500 text-xs mt-0.5">Виден всем в разделе «Публичные». Только просмотр и копирование</p>
            </div>
            <div className={`w-4 h-4 rounded-full border-2 ml-auto mt-1 flex-shrink-0 ${
              visibility === "public" ? "border-green-500 bg-green-500" : "border-zinc-600"
            }`} />
          </button>
          ...
          {onOpenCoverModal && (
            <div className="pt-2 border-t border-white/5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-white">Обложка процесса</p>
                  <p className="text-zinc-500 text-[11px]">
                    {workflow?.thumbnail ? "Обложка установлена" : "Стандартная иконка"}
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  {workflow?.thumbnail ? "Изменить" : "Добавить"}
                </button>
              </div>
            </div>
          )}
