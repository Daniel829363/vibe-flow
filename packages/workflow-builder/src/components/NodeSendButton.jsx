  ...
  return (
    <div className="relative">
      <button
        ...
        title={t("nodes.sendToConnected", {}, "Send to Connected Node")}
      >
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-[#1a1b1e] border border-white/10 rounded-lg shadow-xl overflow-hidden z-50 min-w-max">
          {(() => {
            ...
            return connectedEdges.map((edge) => (
              ...
              >
                {t("nodes.sendTo", { target: edge.target }, `Send to ${edge.target}`)} {targetCounts[edge.target] > 1 ? `(${edge.targetHandle})` : ""}
              </button>
            ));
          })()}
        </div>
